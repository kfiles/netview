...
public class CalendarGadget extends Gadget
  ...
  protected CalendarGadget ( Class c )
    ...
    Date lastOfMonth = new Date( firstOfNextMonth.getTime()
      - ONE_DAY_MS );
    Date startLastMonth = new Date( firstOfMonth.getTime()
      - ONE_DAY_MS * firstOfMonth.getDay() );

    // last month
    int row = 1;
    for ( int day = 0; day < firstOfMonth.getDay(); ++day )
      {
      calendar.setText( row, day, Integer.toString( startLastMonth.getDate()
        + day ) );
      calendar.getCellFormatter()
        .setStyleName( row, day, "Calendar-OtherMonth" );
      }
    ...
    for ( int date = 0; date < lastOfMonth.getDate(); ++date, ++day )
      ...
      calendar.getCellFormatter().setStyleName( row, day,
    ...
    // set style for today
    calendar.getCellFormatter().addStyleName(
      ( firstOfMonth.getDay() + now.getDate() ) / 7 + 1, now.getDay(),
      "Calendar-Today" );
    }
  }
