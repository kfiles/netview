...
public class CookieStorage extends Storage
  ...
  protected void setValuesFromString ( String valueString )
    ...
    if ( valueString.length() > 0 )
      {
      DelimitedStringDeserializer desrializer = new DelimitedStringDeserializer(
        valueString );
      if ( desrializer.hasMore() )
        {
        int size = desrializer.readInt();
        for ( int i = 0; i < size
          && desrializer.hasMore(); ++i )
          {
  ...
  private static CookieConstants constants = (CookieConstants) GWT
    .create( CookieConstants.class );

  public int getTotalStorage ()
    {
    return constants.maxTotalStorage();
    }
  ...
  public void save () throws StorageException
    ...
    for ( int i = 0; i < constants.maxCookies(); ++i )
      Cookies.setCookie( constants.cookiePrefix()
        + i, "", expires );

    // get the values as a string and check their length
    String valueString = getValuesAsString();
    if ( valueString.length() > constants.maxTotalStorage() ) { throw new StorageException(
    ...
    int cookiesRequired = ( (int) valueString.length() / constants
      .maxCookieLength() ) + 1;
    for ( int cookieNum = 0; cookieNum < cookiesRequired; ++cookieNum )
      {
      int begin = cookieNum
        * constants.maxCookieLength();
      int end = Math.min( begin
        + constants.maxCookieLength(), valueString.length() );
      String cookieValue = valueString.substring( begin, end );
      Cookies.setCookie( constants.cookiePrefix()
        + cookieNum, cookieValue, expires );
      }
    }
  ...
  public void load ()
    ...
    String valueString = "";
    String cookieValue = Cookies.getCookie( constants.cookiePrefix()
      + "0" );
    for ( int cookieNum = 0; cookieValue != null
      && cookieValue.length() > 0; )
      {
      valueString += cookieValue;
      ++cookieNum;
      cookieValue = Cookies.getCookie( constants.cookiePrefix()
        + cookieNum );
      }
