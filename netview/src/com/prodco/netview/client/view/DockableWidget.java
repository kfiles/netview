...
public class DockableWidget extends SimplePanel implements MouseListener{
	...
	public void onMouseMove(Widget sender, int x, int y){
	    if( dragInfo != null && switchWith == null ){
	    	int left = x + dragInfo.dragWidget.getAbsoluteLeft() - dragInfo.dragStartX;
	    	int top = y + dragInfo.dragWidget.getAbsoluteTop() - dragInfo.dragStartY;
	...
	protected void findDropContainer( int absCenterX, int absCenterY ){
		...
		for( Iterator itContainers = dockableContainers.iterator();
			 itContainers.hasNext() && switchWith == null;){
			...
			if( containerWidget instanceof HasWidgets &&
				...
				absCenterX <= containerWidget.getAbsoluteLeft() + containerWidget.getOffsetWidth() &&
	...
	protected void findDropWidget( int absCenterX, int absCenterY, HasWidgets container ){
    	int width = dragInfo.dragWidget.getOffsetWidth();
    	int height = dragInfo.dragWidget.getOffsetHeight();
		for( Iterator it = container.iterator();
	 	 it.hasNext() && switchWith == null;){
			Widget sibling = (Widget)it.next();
			if( sibling instanceof DockableWidget && 
				sibling != this &&
				absCenterX >= sibling.getAbsoluteLeft() && 
				absCenterX <= sibling.getAbsoluteLeft() + width &&
				absCenterY >= sibling.getAbsoluteTop() && 
				absCenterY <= sibling.getAbsoluteTop() + height 
				){
