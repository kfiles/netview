...
public class FlowDataDAO
  ...
  public XYDataSet getTopResult ( int siteId, int startTime, int endTime,
    ...
    try
      ...
      if ( null == recs )
        {
        log.warning( "No records matched the query for siteId "
          + siteId );
        return null;
        }
      ...
      if ( recs.size() == 0 )
        ...
        System.out.println( "Got "
          + rpb + " records back from database" );
        }
  ...
  @SuppressWarnings ( "unchecked" )
  public List<JSONObject> getTreemapJsonSet ( int siteId, int startTime,
    int endTime, int num )
    {
    List<JSONObject> rv = new ArrayList<JSONObject>();
    try
      {
      System.out.println( "Getting TreemapDataSet from database for site="
        + siteId );
      ...
      try
        ...
        String sql = "Select flow_id,app_id,app_name,src_ip,src_port,dest_ip,dest_port,"
          + "byte_count,packet_count,site_id from flow_sign where site_id="
          + siteId;
      ...
      // PersistenceManager pm = PersistenceManagerHelper.getInstance()
      // .getPersistenceManager();
      // // String query = "select from " + FlowRecord.class.getName() +
      // " where appName=='SNMP'";
      // // List<FlowRecord> recs = (List<FlowRecord>)
    ...
    try
      ...
      for ( int i = 0; i < num
        ...
        for ( FlowRecord rec : entry.getRecords() )
          {
          String x = labelFormat
            .format( new Date( 30000l * rec.getTimecode() ) );
          if ( x == null )
        ...
        System.out.println( "adding series for property="
          + entry.getTopProp() + " with " + entry.getRecords().size()
          + " records" );
